...
function ProductCard({ product }: any) {
    ...
    return (
      <>
        <div className="flex mb-4">
          <div
            ref={primaryColorRef}
            className=" z-20 w-6 h-6 rounded-2xl"
          ></div>
          <div
            ref={secondaryColorRef}
            className=" w-6 h-6 rounded-2xl -translate-x-1/2"
          ></div>
          <div
            className={
              ' text-sm text-blue-900  items-center text-center ' +
              ''.concat(themeFont.className)
            }
          >
            {product.primaryColor.split('#')['0']} /{' '}
            {product.secondaryColor.split('#')['0']}
          </div>
        </div>
        <div
          className={
            ' mb-4 bg-white text-blue-900 rounded-xl px-2' +
            ' '.concat(themeFont.className)
          }
        >
          {product.price} dhs
  ...
  return (
    <div
      className="flex relative border-[1px]"
    ...
    >
      ...
      {isHovered && (
        <>
          <div
            className={
              'z-10 animate-slidedown absolute mt-2 left-1/2 text-blue-900 text-xl' +
              ' text-bold w-full -translate-x-1/2 px-10 text-center	 '.concat(
                themeFont.className
              )
            }
          >
